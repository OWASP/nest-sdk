...
func newEvents(rootSDK *Nest, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *Events {
	return &Events{
		rootSDK:          rootSDK,
...
func (s *Events) GetEvent(ctx context.Context, eventID string, opts ...operations.Option) (*operations.GetEventResponse, error) {
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/api/v0/events/{event_id}", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out components.EventDetail
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}
