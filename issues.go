...
func newIssues(rootSDK *Nest, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *Issues {
	return &Issues{
		rootSDK:          rootSDK,
...
func (s *Issues) ListIssues(ctx context.Context, request operations.ListIssuesRequest, opts ...operations.Option) (*operations.ListIssuesResponse, error) {
	...
	res := &operations.ListIssuesResponse{
...
func (s *Issues) GetIssue(ctx context.Context, organizationID string, repositoryID string, issueID int64, opts ...operations.Option) (*operations.GetIssueResponse, error) {
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/api/v0/issues/{organization_id}/{repository_id}/{issue_id}", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out components.IssueDetail
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}
