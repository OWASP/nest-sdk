...
```go
nest.WithServerURL("https://api.example.com")
```
...
```go
nest.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
nest.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
nest.WithTimeout(30 * time.Second)
```
